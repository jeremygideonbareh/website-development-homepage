...
const HorizonHero = () => {
  ...
  useEffect(() => {
    ...
    const createNebula = () => {
      ...
      const material = new THREE.ShaderMaterial({
        uniforms: {
          ...
          color2: { value: new THREE.Color(0x444444) },
          opacity: { value: 0.15 },
        },
        vertexShader: `
          varying vec2 vUv;
    ...
    const createMountains = () => {
      ...
      const layers = [
        ...
        { distance: -100, height: 80, color: 0x444444, opacity: 1 },
        { distance: -150, height: 100, color: 0x333333, opacity: 1 },
        { distance: -200, height: 120, color: 0x222222, opacity: 1 },
      ];
    ...
    const animate = () => {
      ...
      refs.mountains.forEach((mountain, i) => {
        ...
        if (mountain.userData.targetZ !== undefined) {
          mountain.position.z += (mountain.userData.targetZ - mountain.position.z) * 0.03;
        }
      });

      if (refs.nebula) {
        refs.nebula.position.z += (refs.nebulaTargetZ - refs.nebula.position.z) * 0.03;
      }
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const refs = threeRefs.current;
      if (!refs.camera || !refs.mountains.length) return;
      ...
      const parallaxDepth = 600;
      refs.mountains.forEach((mountain, i) => {
        const speedFactor = 1 + i * 0.9;
        mountain.userData.targetZ = mountain.userData.baseZ + progress * parallaxDepth * speedFactor;
      });

      refs.nebulaTargetZ = -1050 + progress * 400;
    };
          ...
          <section key={i} className="content-section">
            ...
            <div className="hero-subtitle cosmos-subtitle">
              {section.subtitle.map((line, j) => (
                <p key={j} className="subtitle-line">
                  {line}
                </p>
              ))}
            </div>
